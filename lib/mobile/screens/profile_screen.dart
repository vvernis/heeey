    ...
    return Scaffold(
      appBar: AppBar(
        ...
        automaticallyImplyLeading: false,
         actions: [
      IconButton(
        icon: const Icon(Icons.settings, color: lightGray),
        onPressed: () {
...
Widget _buildHeader(Map<String, dynamic> data, String uid) {
  ...
  final String startYearStr = data['startYear'] ?? '0';
  final String gradYearStr  = data['graduationYear'] ?? '0';

  // Safely parse them as integers
final int sYear = int.tryParse(startYearStr) ?? 0;
final int gYear = int.tryParse(gradYearStr) ?? 0;

// Build the AY string only if both are non-zero
String ayText = '';
if (sYear > 0 && gYear > 0) {
  // For example, 2022 -> 22, 2023 -> 23
  final int sShort = sYear % 100; 
  final int gShort = gYear % 100;
  ayText = 'AY$sShort/$gShort';
}

// If AY is not empty, we append it in parentheses after the course name
final String courseDisplay = ayText.isNotEmpty 
    ? "$course [$ayText]"
    : course;
